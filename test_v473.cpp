...
{
    printf("VME Bust Test\n");

    uint16_t const data_pattern[10] = {
	0x0000, 0xFFFF, 0x00FF, 0xFF00, 0x0F0F,
	0xF0F0, 0x3333, 0xCCCC, 0x5555, 0xAAAA
    };
    ...
    printf("Testing VME -> Dual Port Data Bus...\n");

    for (size_t index = 0; index < 10; index++)
	// Write to <base address> + 2 * index

	sysOut16(dataBuffer + index, data_pattern[index]);

    for (size_t index = 0; index < 10; index++) {
	// Read from <base address> + 2 * index

	receivedData = sysIn16(dataBuffer + index);

	printf("Address 0x%04X:  Wrote 0x%04X, Read 0x%04X", index * 2,
	       data_pattern[index], receivedData);
    ...
    printf("Testing VME -> Dual Port Address Bus, Walking 1...\n");
	...
	printf("Address 0x%04X:  Wrote 0x%04X, Read 0x%04X", (0x0002 << index),
    ...
    printf("Testing VME -> Dual Port Address Bus, Walking 0...\n");
	...
	printf("Address 0x%04X:  Wrote 0x%04X, Read 0x%04X",
	       ~(0x0002 << index) & 0x00007FFE, expectedData, receivedData);
	...
	// Overwrite data pattern to verify CPU really processed it

	sysOut16(dataBuffer, ~data_pattern[index]);
    ...
    printf("Testing FPGA Automatic Pattern\n");

    // One write to prime the pattern

    hw->setVmeDataBusDiag(lock, &data_pattern[0]);
    ...
    if (testPass) {
	printf("V473 VME Bus Test PASSED\n");
	return 0;
    } else {
	printf("V473 VME Bus Test FAILED\n");
	return 1;
    }
...
int TestDiscIO(V473::HANDLE const hw)
{
    ...
    printf("Testing V473 Discrete I/O\n");
    ...
    // Give card a chance to change outputs and update status
	...
	for (channel_read = 0; channel_read < 4; channel_read++) {
		...
		printf("DiscIO Error, Reading Channel %i\n", channel_read);
	...
	// The Reset output is run by the processor's 1sec interrupt
	// cycle. It may take up to a second to see the output change.

        hw->getPowerSupplyStatus(lock, channel_drive, &ps_stat_received);

	// Sniff the PS Reset status bit

        while ((ps_stat_received & 0x2000) != 0x2000)
            hw->getPowerSupplyStatus(lock, channel_drive, &ps_stat_received);
	...
	// Sniff the PS Reset status bit
        ...
        while ((ps_stat_received & 0x2000) != 0x0000)
            hw->getPowerSupplyStatus(lock, channel_drive, &ps_stat_received);
    ...
    if (testPass) {
        printf("V473 Discrete I/O Test PASSED\n");
        return 0;
    } else {
	printf("V473 Discrete I/O Test FAILED\n");
	return 1;
    }
...
int Calibrate(V473::HANDLE const hw)
{
    ...
    printf("V473 Calibration\n");
    printf("Press any key to continue\n");

    // Skips over extra line feeds and other characters

    while (!isalnum(kb_input) && kb_input != ' ')
        scanf("%c", &kb_input);
    ...
    while (1) {
        printf("Setting all DAC Channels to 0V\n");
	...
	// Skips over extra line feeds and other characters
        ...
        printf("Setting all DAC Channels to +10V\n");
        for (channel = 0; channel < 4; channel++)
...
int TestAnalogIO(V473::HANDLE const hw)
{
    ...
    printf("Testing V473 Analog I/O\n");
	...
	if (ADCvalue_signed < -tolerance_0V ||
	    ADCvalue_signed > tolerance_0V) {
	    printf("Analog Readback Error on channel %i:\n", channel);
	...
	if (ADCvalue_signed < 31130 - tolerance_9_5V ||
	    ADCvalue_signed > 31130 + tolerance_9_5V) {
	    printf("Analog Readback Error on channel %i:\n", channel);
	...
	if (ADCvalue_signed < 16334 - tolerance_5V ||
	    ADCvalue_signed > 16434 + tolerance_5V) {
	    printf("Analog Readback Error on channel %i:\n", channel);
    ...
    if (testPass) {
    	printf("V473 Analog I/O Test PASSED\n");
        return 0;
    } else {
    	printf("V473 Analog I/O Test FAILED\n");
        return 1;
    }
}
